import type { EmojiItem } from './baseEmoji';

// Новые emoji для категории "Технологии"
export const techEmoji: EmojiItem[] = [
    { emoji: '📱', keywords: ['phone', 'телефон', 'мобильный', 'смартфон'], category: 'tech' },
    { emoji: '💻', keywords: ['laptop', 'ноутбук', 'компьютер', 'лаптоп'], category: 'tech' },
    { emoji: '🖥️', keywords: ['desktop', 'компьютер', 'монитор', 'ПК'], category: 'tech' },
    { emoji: '⌚', keywords: ['watch', 'часы', 'умные часы', 'годинник'], category: 'tech' },
    { emoji: '📷', keywords: ['camera', 'камера', 'фото', 'фотоаппарат'], category: 'tech' },
    { emoji: '🎧', keywords: ['headphones', 'наушники', 'музыка', 'навушники'], category: 'tech' },
    { emoji: '📺', keywords: ['tv', 'телевизор', 'экран', 'телевізор'], category: 'tech' },
    { 
        emoji: '�️', 
        keywords: ['printer', 'принтер', 'печать', 'printing', 'друк', 'документы'], 
        category: 'tech' 
    },
    { 
        emoji: '💽', 
        keywords: ['disk', 'диск', 'хранение', 'storage', 'данные', 'компьютер'], 
        category: 'tech' 
    },
    { 
        emoji: '🔌', 
        keywords: ['plug', 'розетка', 'электричество', 'power', 'зарядка', 'energy'], 
        category: 'tech' 
    }
];